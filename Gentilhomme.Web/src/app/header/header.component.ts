...
export class HeaderComponent implements OnInit {
  ...
  openModal(name: string) {
    ...
    if (name === 'login') {
      let Login = this.componentFactoryResolver.resolveComponentFactory(LoginComponent);
      this.container.createComponent(Login);
      this.modalTitle.nativeElement.textContent = 'Enter Login Information';
    }
    if (name === 'register') {
      let Register = this.componentFactoryResolver.resolveComponentFactory(RegisterComponent);
      this.container.createComponent(Register);
